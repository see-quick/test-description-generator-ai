...
public class NetworkPoliciesST extends AbstractST {
    ...
    @TestDoc(description = @Desc("This test case verifies network policies if Cluster Operator is in the same namespace as Operands."), contact = @Contact(name = "see-quick", email = "maros.orsak159@gmail.com"), steps = { @Step(value = "Deploy Kafka with plain and TLS listeners, having set network policies for respective clients", expected = "Kafka is deployed"), @Step(value = "Deploy KafkaTopics, metric scraper Pod, and all Kafka clients", expected = "All resources are deployed"), @Step(value = "Produce and consume messages with clients using respective listeners", expected = "Clients fail or succeed in connecting to Kafka based on specified network policies"), @Step(value = "Scrape exported metrics regarding present Kafka Topics", expected = "Metrics are present and have expected values"), @Step(value = "Change log configuration in Kafka config", expected = "Observed generation is higher than before the config change and Rolling Update did not take place") }, useCases = { @UseCase(id = "network-policy"), @UseCase(id = "listeners"), @UseCase(id = "metrics") }, tags = { @TestTag(value = "default"), @TestTag(value = "regression"), @TestTag(value = "network"), @TestTag(value = "kafka") })
    ...
    @IsolatedTest("Specific Cluster Operator for test case")
    @TestDoc(description = @Desc("Test verifying the behavior of the Cluster Operator when it is deployed in a different namespace than the Kafka operand."), contact = @Contact(name = "see-quick", email = "maros.orsak159@gmail.com"), steps = { @Step(value = "Assume the environment is not using Helm or OLM for installation", expected = "Condition is verified"), @Step(value = "Initialize test storage and define a secondary namespace", expected = "Test storage is set and secondary namespace is defined"), @Step(value = "Create and configure Cluster Operator environment variables", expected = "Cluster Operator environment variables are configured"), @Step(value = "Run the Cluster Operator installation with the specified namespaces", expected = "Cluster Operator is installed across specified namespaces"), @Step(value = "Edit the namespace to add custom labels", expected = "Namespace is successfully updated with new labels"), @Step(value = "Set the secondary namespace for cluster operations", expected = "Secondary namespace is set"), @Step(value = "Create resources for Kafka Node Pools", expected = "Kafka Node Pools are created with persistent storage"), @Step(value = "Create Kafka resources with metrics configuration", expected = "Kafka resources with metrics configuration are deployed"), @Step(value = "Verify network policies in the secondary namespace", expected = "Network policies are verified successfully"), @Step(value = "Change Kafka configuration and verify observed generation changes", expected = "Kafka configuration changes are applied and verified") }, useCases = { @UseCase(id = "namespace-separation"), @UseCase(id = "cluster-operator-multi-namespace"), @UseCase(id = "kafka-deployment") }, tags = { @TestTag(value = "namespace"), @TestTag(value = "kafka"), @TestTag(value = "cluster-operator") })
    @TestDoc(description = @Desc("Test verifying the behavior of the Cluster Operator when it is deployed in a different namespace than the Kafka operand."), contact = @Contact(name = "see-quick", email = "maros.orsak159@gmail.com"), steps = { @Step(value = "Assume the environment is not using Helm or OLM for installation", expected = "Condition is verified"), @Step(value = "Initialize test storage and define a secondary namespace", expected = "Test storage is set and secondary namespace is defined"), @Step(value = "Create and configure Cluster Operator environment variables", expected = "Cluster Operator environment variables are configured"), @Step(value = "Run the Cluster Operator installation with the specified namespaces", expected = "Cluster Operator is installed across specified namespaces"), @Step(value = "Edit the namespace to add custom labels", expected = "Namespace is successfully updated with new labels"), @Step(value = "Set the secondary namespace for cluster operations", expected = "Secondary namespace is set"), @Step(value = "Create resources for Kafka Node Pools", expected = "Kafka Node Pools are created with persistent storage"), @Step(value = "Create Kafka resources with metrics configuration", expected = "Kafka resources with metrics configuration are deployed"), @Step(value = "Verify network policies in the secondary namespace", expected = "Network policies are verified successfully"), @Step(value = "Change Kafka configuration and verify observed generation changes", expected = "Kafka configuration changes are applied and verified") }, useCases = { @UseCase(id = "namespace-separation"), @UseCase(id = "cluster-operator-multi-namespace"), @UseCase(id = "kafka-deployment") }, tags = { @TestTag(value = "namespace"), @TestTag(value = "kafka"), @TestTag(value = "cluster-operator") })
    ...
    @TestDoc(description = @Desc("Test ensuring that no NetworkPolicies are generated by Strimzi when the STRIMZI_NETWORK_POLICY_GENERATION environment variable is set to false."), contact = @Contact(name = "see-quick", email = "maros.orsak159@gmail.com"), steps = { @Step(value = "Assume the test environment is not set for Helm or OLM installation", expected = "The test proceeds only if the assumption is true"), @Step(value = "Set the environment variable STRIMZI_NETWORK_POLICY_GENERATION to false", expected = "The environment variable is configured correctly"), @Step(value = "Install the Cluster Operator with the specified environment variable", expected = "The Cluster Operator is installed successfully"), @Step(value = "Create Kafka NodePool resource", expected = "Kafka NodePool resource is created successfully"), @Step(value = "Create Kafka resource with Cruise Control", expected = "Kafka resource with Cruise Control is created successfully"), @Step(value = "Create Kafka Connect resource", expected = "Kafka Connect resource is created successfully"), @Step(value = "Check for NetworkPolicies generated by Strimzi", expected = "No NetworkPolicies are generated") }, useCases = { @UseCase(id = "network-policy-generation-disabled"), @UseCase(id = "cruise-control-validation") }, tags = { @TestTag(value = "CRUISE_CONTROL"), @TestTag(value = "network-policy") })
